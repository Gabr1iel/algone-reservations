# Zadání semestrálního projektu — Hotelový rezervační systém „Algone Reservations"
...
## Povinné databázové objekty

- **VIEW** — v_active_reservations: přehled aktuálních rezervací s joinem na uživatele, pokoje a hotely.
- **FUNCTION** — fn_calculate_total_price: výpočet celkové ceny rezervace z počtu nocí a ceny pokoje.
- **PROCEDURE** — sp_cancel_expired_reservations: automatické zrušení PENDING rezervací starších než stanovený počet dní.
- **TRIGGER** — trg_reservation_updated_at: automatická aktualizace sloupce updated_at při každé změně záznamu rezervace.
- **Transakce** — vytvoření rezervace probíhá v transakci s kontrolou dostupnosti pokoje a rollbackem při kolizi.
